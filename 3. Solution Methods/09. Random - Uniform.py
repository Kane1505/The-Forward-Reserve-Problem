# -*- coding: utf-8 -*-
"""
Created on Sun Mar 10 13:21:24 2024
...
# Remove random SKUs, such that 'Number_of_SKUs' SKUs remain
remove_index = np.random.choice(range(len(SKUs)), len(SKUs) - Number_of_SKUs, replace=False)

s = np.delete(s, remove_index, axis = 0)
p = np.delete(p, remove_index)
dl = np.delete(dl, remove_index)
du = np.delete(du, remove_index)
psi = np.delete(psi, remove_index, axis = 0)
theta = np.delete(theta, remove_index)
SKUs = np.delete(SKUs, remove_index)
...
# Initialize a matrix that stores the previous solution
prev_sol = np.zeros([len(SKUs), len(c)])
...
while np.array_equal(prev_sol,cur_sol) == False and sum(not_reached_max_stock) != 0:
    ...
    # For each SKU that has not yet reached highest capacity, give it an equal fraction of the spaces available
    space_allocation_fraction = np.ones(len(not_reached_max_stock))/sum(not_reached_max_stock) * not_reached_max_stock
...
results.to_excel('Assignment and Allocation Heuristic 9.xlsx', index = False)
results.to_pickle('Assignment and Allocation Heuristic 9.pkl')
