# -*- coding: utf-8 -*-
"""
Created on Sat Mar  9 13:05:32 2024
...
# Import all data
p = pd.read_pickle('Mean Daily Picks Normal 1000.pkl')
s = pd.read_pickle('Max Units per Location Type 1000.pkl')
psi = pd.read_pickle('Safety Stock in Locations Empirical 99 Normal 1000.pkl')
demand = pd.read_pickle('Normal Demand 1000.pkl')
theta = pd.read_pickle('Maximum Inventory Normal 1000.pkl')
...
while iteration < len(du):
    ...
    max_stock = np.zeros(iteration+1)
    ...
    while S_prime > 0:
        ...
        for j in M:
            ...
            if S_prime >= c_prime[j+1] and S_prime < c_prime[j]:
                ...
                best = np.argmin(np.concatenate((np.zeros([iteration+2,j]), v[:,j:]), axis = 1), axis = None)
            ...
            # If there is not enough space left to fill the smallest location, terminate this iteration
                ...
                S_prime = -10
                break
    # Once all the space has been filled, calculate the amount of replenishments needed
...
results.to_excel('Assignment and Allocation Heuristic 1.xlsx', index = False)
results.to_pickle('Assignment and Allocation Heuristic 1.pkl')
