# -*- coding: utf-8 -*-
"""
Created on Fri Mar  8 15:56:59 2024
...
units_per_location = pd.read_pickle('Max Units per Location Type Exponential 10000.pkl')
...
for i in range(len(demand)):
    # ... order the demand
    ordered_demand = np.sort(demand[i])
    # Find which observation crosses the alpha threshold
    critical_obs = math.ceil(len(demand[i])*alpha)
    # Set safety stock units equal to the critical observation
    safety_stock_units[i] = ordered_demand[critical_obs-1]
...
safety_stock_units.to_excel('Safety Stock in Units Exponential 10000.xlsx', index = False)
safety_stock_units.to_pickle('Safety Stock in Units Exponential 10000.pkl')
...
# Save the data to Excel and to Pickle  
safety_stock_locations.to_excel('Safety Stock in Locations Empirical 99 Exponential 10000.xlsx', index = False)
safety_stock_locations.to_pickle('Safety Stock in Locations Empirical 99 Exponential 10000.pkl')